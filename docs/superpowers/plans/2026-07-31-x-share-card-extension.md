...
> **計畫外補做（2026-08-01）**：本計畫原先漏掉 spec §7 的**手動編輯模式**，
> 而 spec §4.0 把它列為「X 停止對未登入請求提供 microdata」這個殘餘風險的
> 唯一緩解手段。已實作於 `src/editor/manual.ts` 與 `Panel.tsx` 的錯誤狀態。
> 同時補上 spec §7 承諾的「重試」按鈕，以及固定比例裁切的漸層淡出。
