...
export type Post = {
  ...
  title?: string        // 小紅書筆記標題，與內文分屬兩段。階段 3 才加，見「階段 3 是可拋棄的」
  rawText: string
  text: Segment[]
...
`export.ts` 的 `MAX_EXPORT_PIXELS`（1600 萬）擋的是 iOS Safari 超過該像素數會
**給出全白畫布而不丟例外**的行為 —— 無聲失敗，所以 `exportScale` 撞到上限時會
整體縮小而不是照原倍率輸出。

過去有字數上限擋著，畫布高度到不了那個量級，這條防線形同備而不用。移除字數
上限後它會真的被觸發：1080 寬對應的高度上限是 **14,814px**。超過之後匯出的圖
會悄悄變成不到 1080 寬 —— 不是壞掉，但與「輸出寬度固定 1080」這個承諾不符，
而使用者看不出來。

處理方式：`exportScale` 回傳值低於「寬度剛好 1080」所需的倍率時，UI 要明講
這張圖因為過長而低於標準寬度。不改變縮小行為本身（那是對的，無聲全白更糟），
只是不讓它默默發生。這條路徑要有測試 —— 它在真實使用中極少觸發，沒有測試就
等於沒人會發現它壞了。
...
微博與小紅書是**先試，不成就放棄**。兩者都只能讀 DOM，對方改一次版就壞，
維護成本是持續性的而非一次性的。若實作過程中 bug 過多或關鍵環節無解
（最可能是圖片 CDN 的存取限制），就砍掉這兩個平台，產品收斂為 X + Threads。

這個前提對前兩階段有約束，必須在設計時就守住：

**階段 1 與 2 不得以微博／小紅書為理由做任何抽象。** 每一項通用化都要能只用
X 與 Threads 說明白：

| 改動 | 只有 X + Threads 時還成立嗎 | 落在哪一階段 |
| --- | --- | --- |
| `metrics` 有序陣列 | 成立 —— Threads 有 `shares`，X 沒有；X 有 `views`，Threads 沒有 | 階段 1 |
| `textComplete` | 成立 —— X 引用推文超過 200 字就是截斷 | 階段 1 |
| adapter 介面 | 成立 —— X 免 cookie 抓取與 Threads 經代理已是兩種擷取 | 階段 1 |
| `handleDisplay` | 成立 —— Threads 的 handle 規則與 X 不同 | 階段 1 |
| `CardShell` / `MediaLayout` 拆分 | 成立 —— Threads 與 X 都有多圖貼文 | 階段 2 |
| **`title?` 欄位** | **不成立 —— 只有小紅書筆記有獨立標題** | **移到階段 3** |

`title?` 原本列在階段 1 的模型改動裡，移到階段 3。砍掉小紅書時它會是唯一
需要一併移除的欄位，而不是散落在各處的死程式碼。

砍掉階段 3 的代價因此收斂成：刪兩份 adapter、刪 `title?`、`MetricKind` 拿掉
`bookmarks`、manifest 移除兩組 host_permissions。`MediaLayout` 留著 —— 它對
帶圖的推文與 Threads 貼文本來就有用。
